...
import os
import sys
from HelloWorld.BackEnd  import UserGuide_BE
import pandas as pd
...
def submit_label(request):
    ...
    #Return the first 10 lines of train file
    #---------------------------------------
    #The method of getting the value of 'data_preview_samples'
    #For example: The value of 2nd line with column 'Dates' can be indexed by context['data_preview_samples']['Dates'][1]    
    context['columns'], context['data_preview_samples'] = UG_BE.GetDataPreviewSamples()
    print context['columns']
    #print context['data_preview_samples']['Dates'][1]
...
def save_feature_type(request):
    ...
    UG_BE.TriggerModel()
    #print UG_BE.GetPredictResults()
    UG_BE.WritePredictResults()

    return render(request, 'finished.html', context)

# Ajax, return file's feature list
def feature_list(request, file):
    baseDir = os.path.dirname(os.path.abspath(__name__))
    path = os.path.join(baseDir,'static',file + '.csv')

    try:
        f_list = list(pd.read_csv(path, encoding='u8').columns)
        return JsonResponse({'feature_list': f_list})
